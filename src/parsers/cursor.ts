...
function isCursorNoiseText(
  rawText: string,
  cleanedText: string,
  role: NormalizedCursorLine["role"],
): boolean {
    ...
    for (const projectDir of projectDirs) {
      ...
      const found = findFiles(ctx, transcriptsDir, {
        match: (entry, fullPath) =>
          entry.name.endsWith(".jsonl") && fullPath.includes("agent-transcripts"),
        maxDepth: 2,
      });
      files.push(...found);
    }
  ...
  for (const line of lines) {
    ...
    for (const block of line.content) {
      if (block.type === "text" && block.text) {
        const cleaned =
          line.role === "user"
            ? cleanCursorUserText(block.text)
            : stripCursorMetadataTags(block.text);
